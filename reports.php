...
<nav class="navbar navbar-dark bg-dark main-dashboard-cip other-pages">
    <a class="navbar-brand" href="<?php echo esc_url(admin_url('admin.php?page=clock-in-portal')); ?>"><i
                class="fas fa-home"></i></a>
    <a class="navbar-brand" href="#"><?php esc_html_e('Administrar Informes', CIP_FREE_TXTDM); ?></a>
    <div class="form-inline my-2 my-lg-0">
        <a class="navbar-brand" href="<?php echo esc_url(admin_url('admin.php?page=cip-settings')); ?>"><i
                            ...
                            if (!empty ($all_staffs = $wpdb->get_results($wpdb->prepare("SELECT * FROM `$staff_table` WHERE `status` = %d", 1)))) {
                                foreach ($all_staffs as $staff_data) {
                                    ...
                                    if (!empty($user_info)) {
                                        ...
                                        ?>
                                        <option value="<?php echo esc_attr($staff_id); ?>" <?php if ($userid == $staff_id) echo esc_attr("selected=selected"); ?>><?php esc_html_e($fullname); ?>
...
<!--                        </optgroup>-->
                    ...
                    <button type="submit"
                            class="btn btn-info "><?php esc_html_e('Obtener informe', CIP_FREE_TXTDM); ?></button>
                </td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
                <td>&nbsp;</td>
...
<!--            <th>--><?php //esc_html_e('Finalización de Almuerzo', CIP_FREE_TXTDM); ?><!--</th>-->
        ...
        if ($user_info = get_userdata($userid)) {
            ...
            $headertext = "N., Fecha, Nombre, Entrada, Salida, Descanso, Horas Trabajadas, IP, Lugar \n";
            ...
            foreach ($all_days_report as $row_date) {
                // Fetch all records for this user on this date
                ...
                // -----------------------------------------------------
                // 1) IF $attend_filter_name == "1"
                // -----------------------------------------------------
                if ($attend_filter_name == "1") {

                    if (!empty($rows)) {
                        // We have one or more records for this day
                        foreach ($rows as $row) {
                            $flag = 1;
                            $id         = $row->id;
                            $date       = $row->date;
                            ...
                            // If you need region/city/country logic here, you can add it.

                            // Calculate total work hour if office_out is set
                            ...
                            // Calculate break hour if available
                            ...
                            // If you need lunch_in/lunch_out, uncomment and apply similarly:
                            // $lunch_in  = ($row->lunch_in == "00:00:00") ? "None" : date($time_format, strtotime($row->lunch_in));
                            // $lunch_out = ($row->lunch_out == "00:00:00") ? "None" : date($time_format, strtotime($row->lunch_out));
                            ...
                            <tr>
                                ...
                                <td><?php esc_html_e($date_formatted); ?></td>
                                <td><?php esc_html_e($office_in); ?></td>
                                <td><?php esc_html_e($office_out); ?></td>
                                <!-- Uncomment if you need lunch columns:
                    <td><?php //esc_html_e($lunch_in); ?></td>
                    <td><?php //esc_html_e($lunch_out); ?></td> -->
                                <td><?php esc_html_e($break_hour); ?></td>
                                <td><?php echo esc_attr(($office_in != "None") ? $work_hour : 'None'); ?></td>
                                <td><?php echo esc_attr(($office_in != "") ? $user_ip : "None"); ?></td>
                                <td><?php echo esc_attr(($office_in != "None") ? $row->user_location : "None"); ?></td>
                                <td>
                                    <?php if ($office_in != "None") { ?>
                                        <button id="view-report" name="view-report" class="btn btn-info"
                                                data-toggle="modal" data-target="#ViewModal"
                                                onclick="return ViewReport('<?php echo esc_attr($id); ?>',
                                                        '<?php echo esc_attr($userid); ?>',
                                                        '<?php echo esc_attr($row->date); ?>');">
                                            Ver informe
                                        </button>
                                        <button id="edit-record" name="edit-record" class="btn btn-primary"
                                                data-toggle="modal" data-target="#EditModal"
                                                onclick="return EditRecord('<?php echo esc_attr($id); ?>',
                                                        '<?php echo esc_attr($userid); ?>',
                                                        '<?php echo esc_attr($row->date); ?>');">
                                            Editar
                                        </button>
                                    <?php } else {
                                        echo esc_attr('No report');
                                    } ?>
                                </td>
                            </tr>
                            <?php
                            ...
                            $txt = "$no, $csv_date, $fullname, $office_in, $office_out, $break_hour, $work_hour, $user_ip, $region, $city, $country \n";
                        ...
                        $flag = 1;
                        $date_formatted = date($date_format, strtotime($row_date));
                        $office_in = "-";
                        $office_out = "-";
                        $break_hour = "00:00:00";
                        $work_hour = "-";
                        ...
                        // Write no-record row to CSV
                        $csv_date = date("d M y", strtotime($row_date));
                        $txt = "$no, $csv_date, $fullname, $office_in, $office_out, $break_hour, $work_hour, $user_ip, $region, $city, $country \n";
                        fwrite($report_file, $txt);
                    ...
                    if (!empty($rows)) {
                        foreach ($rows as $row) {
                            ...
                            $office_out = $row->office_out;
                            $user_ip    = $row->ip;
                            // region/city/country logic if needed
                            ...
                            <tr>
                                ...
                                <td><?php esc_html_e($office_in); ?></td>
                                <td><?php esc_html_e($office_out); ?></td>
                                <!-- <td>...</td><td>...</td> if you need lunch columns -->
                                <td><?php esc_html_e($break_hour); ?></td>
                                <td><?php echo esc_attr(($office_in != "None") ? $work_hour : 'None'); ?></td>
                                <td><?php echo esc_attr(($office_in != "") ? $user_ip : "None"); ?></td>
                                <td><?php echo esc_attr(($office_in != "None") ? $row->user_location : "None"); ?></td>
                    ...
                    // -----------------------------------------------------
                    // 3) IF $attend_filter_name == "3"
                    // -----------------------------------------------------
                } elseif ($attend_filter_name == "3") {
                    // Example logic: Only show days with empty records? Or something else?
                    // We'll remove Sunday checks and handle multiple rows similarly.
                    if (empty($rows)) {
                        // No record found for this date
                        $flag = 1;
                        $date_formatted = date($date_format, strtotime($row_date));
                        ...
                        // CSV
                        $csv_date = date("d M y", strtotime($date_formatted));
                        $txt = "$no, $csv_date, $fullname, $office_in, $office_out, $break_hour, $work_hour, $user_ip, $region, $city, $country \n";
                        fwrite($report_file, $txt);
                        $no++;

                    } else {
                        // If there are rows, decide how to handle them. For instance:
                        foreach ($rows as $row) {
                            $flag = 1;
                            $id         = $row->id;
                            $date       = $row->date;
                            ...
                            // Calculate hours, etc., same as above
                            if ($office_out != '00:00:00') {
                                $dteStart = new DateTime($row->office_in);
                                $dteEnd   = new DateTime($row->office_out);
                                $dteDiff  = $dteStart->diff($dteEnd);
                            ...
                            $txt = "$no, $csv_date, $fullname, $office_in, $office_out, $break_hour, $work_hour, $user_ip, $user_location \n";
                            fwrite($report_file, $txt);
...
//end of foreach
    ...
    if (isset($flag) && !empty($flag)) {
        // Existing code that prints the total working hours
        ...
        // =========================
        // NEW: Add a download button for the CSV file
        // Make sure $file_name and $upload_dir_url are in scope here
        // =========================
        echo "<p>";
        echo "<a class='btn btn-success' href='" . esc_url($upload_dir_url . "/" . $file_name) . "' download>";
        echo "<i class='fas fa-download' aria-hidden='true'></i> Descargar Informe";
        echo "</a>";
        echo "</p>";
    }
    ?>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title text-white"
                        id="myModalLabel"><?php esc_html_e('View Record', CIP_FREE_TXTDM); ?></h4>
    ...
    <div class="modal fade" id="EditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg modal-notify modal-info" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title text-white"
                        id="myModalLabel"><?php esc_html_e('Edit Record', CIP_FREE_TXTDM); ?></h4>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" id="update-record" name="update-record" class="btn btn-default"
                            onclick="return UpdateRecoed();"><?php esc_html_e('Update', CIP_FREE_TXTDM); ?></button>
                    <button type="button" class="btn btn-default"
                            data-dismiss="modal"><?php esc_html_e('Close', CIP_FREE_TXTDM); ?></button>
                </div>
            </div>
        </div>
    </div>
...
<!--                    <tr>-->
                    ...
                    <tr>
                        ...
                        <td><?php if ($office_in != "None") {
                                esc_html_e($report_data->user_location);
                            } else {
                                esc_html_e("None");
                            } ?></td>
                    <tr>
            ...
            <div id="edit-report-result">
                <form id="update-record-form" name="update-record-form">
                    <table class="table table-bordered">
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="hidden" id="staff_id" name="staff_id"
                                       value="<?php echo esc_attr($staff_id); ?>">
                        ...
                        <tr>
                            <th><?php esc_html_e('Office In', CIP_FREE_TXTDM); ?></th>
                            <td><input type="text" class="datetimepicker2" id="office_in" name="office_in"
                                       value="<?php echo esc_attr($office_in); ?>"></td>
                        <tr>
                        <tr>
                            <th><?php esc_html_e('Office Out', CIP_FREE_TXTDM); ?></th>
                            <td><input type="text" class="datetimepicker3" id="office_out" name="office_out"
                                       value="<?php echo esc_attr($office_out); ?>"></td>
                        <tr>
<!--                        <tr>-->
<!--                            <th>--><?php //esc_html_e('Lunch In', CIP_FREE_TXTDM); ?><!--</th>-->
<!--                            <td><input type="text" class="datetimepicker4" id="lunch_in" name="lunch_in"-->
<!--                                       value="--><?php //echo esc_attr($lunch_in); ?><!--"></td>-->
<!--                        <tr>-->
<!--                        <tr>-->
<!--                            <th>--><?php //esc_html_e('Lunch Out', CIP_FREE_TXTDM); ?><!--</th>-->
<!--                            <td><input type="text" class="datetimepicker5" id="lunch_out" name="lunch_out"-->
<!--                                       value="--><?php //echo esc_attr($lunch_out); ?><!--"></td>-->
<!--                        <tr>-->
                        <tr>
                            <th><?php esc_html_e('Working Hours', CIP_FREE_TXTDM); ?></th>
                            <td><input type="text" class="datetimepicker5" id="today_total_hours"
                                       name="today_total_hours" value="<?php if ($office_in != "00:00:00") {
                                    echo esc_attr($work_hour);
                                } else {
                                    echo esc_attr("None");
                                } ?>"></td>
                        <tr>
                        <tr>
                            ...
                            <td><input type="text" id="user_ip" name="user_ip"
                                       value="<?php echo esc_attr($report_data->ip); ?>"></td>
                        <tr>
                        <tr>
                            <th><?php esc_html_e('Location', CIP_FREE_TXTDM); ?></th>
                            <td><input type="text" id="location" name="location"
                                       value="<?php if ($office_in != "None") {
